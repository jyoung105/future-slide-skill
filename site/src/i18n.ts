...
const en = {
  ...
  "examples.sub": "Drop a reference image, point at a PDF, and let each skill stay in its lane.",
...
const ko = {
  ...
  "why.p5": "참조 이미지의 슬라이드 디자인 시스템이 아닌, 보이는 텍스트에 과적합됩니다.",
  "why.p6": "프롬프트 작성에서 멈추고, 번호 매겨진 슬라이드를 절대 렌더링하지 않습니다.",
  ...
  "examples.sub": "참조 이미지를 던지고, PDF를 가리키고, 각 스킬이 자기 영역에 머물도록 두세요.",
...
export function applyLocale(locale: Locale) {
  ...
  const flowDiagram = getElement<HTMLImageElement>("flow-diagram");
  if (flowDiagram) {
    flowDiagram.src = SITE_META.diagram[locale];
    flowDiagram.alt = I18N[locale]["flow.alt"];
  }
